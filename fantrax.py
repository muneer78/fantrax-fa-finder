...
def _chadwick_name(name: str) -> str:
    """Normalize a Fantrax player name for Chadwick lookup.

    Fantrax getPlayerIds returns names as "Last, First" (e.g. "Burger, Jake").
    Chadwick stores them as "first last" lowercase (e.g. "jake burger").
    Also strips Jr/Sr/II/III suffixes that Chadwick omits.
    """
    name = name.strip()
    if "," in name:
